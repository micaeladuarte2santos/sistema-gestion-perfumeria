...
public class EmailServiceImpl implements IEmailService {
    ...
    @Override
    public void enviarCodigoRecuperacion(String destinatario, String codigo, String nombreUsuario) {
        try {
            MimeMessage mensaje = mailSender.createMimeMessage();
            MimeMessageHelper helper = new MimeMessageHelper(mensaje, true, "UTF-8");

            helper.setTo(destinatario);
            helper.setSubject("Recuperacion de Contrasena - Perfumeria Flowers");

            String contenidoHtml = String.format("""
                <html>
                <body style="font-family: Arial, sans-serif; background-color: #f4f4f4; padding: 20px;">
                    <div style="max-width: 600px; margin: 0 auto; background-color: white; padding: 30px; border-radius: 10px; box-shadow: 0 2px 5px rgba(0,0,0,0.1);">
                        <h2 style="color: #333; text-align: center;">Recuperacion de contrasena</h2>
                        <p style="color: #666; font-size: 16px;">Hola <strong>%s</strong>,</p>
                        <p style="color: #666; font-size: 16px;">
            ...
            throw new RuntimeException("Error al enviar correo de recuperacion: " + e.getMessage());
        }
    }
}
